...
class IpoptProblemWrapper(object):
    ...
    def objective(self, x):
        ...
        if self.traj_iter is not None:
            if self.nit % self.traj_iter == 0:
               self.traj.append([self.nit, x])
        return self.fun(x, *self.args, **self.kwargs)

    def gradient(self, x, **kwargs):
        self.njev += 1
        jacco = self.jac(x, *self.args, **self.kwargs)

        if self.traj_iter is not None:
            if self.nit % self.traj_iter == 0:
               self.grad_list.append(jacco)
    ...
    def intermediate(self, alg_mod, iter_count, obj_value,
                     ...
                     alpha_pr, ls_trials):
        self.nit = iter_count
        self.loss_list.append(obj_value)
...
def minimize_ipopt(fun, x0, args=(), kwargs=None, method=None,
    ...
    problem = IpoptProblemWrapper(
        ...
        constraints=constraints, eps=1e-8,
        con_dims=con_dims, traj=traj,
        traj_iter=traj_iter, loss_list=loss_list,
        grad_list=grad_list)
    ...
    return OptimizeResult(
        x=x, success=info['status'] == 0,
        status=info['status'], message=info['status_msg'],
        fun=info['obj_val'], info=info,
        nfev=problem.nfev, njev=problem.njev,
        nit=problem.nit), problem.traj,problem.loss_list, problem.grad_list
...
class ObjectiveWrapper:
    ...
    def __init__(self, func):
        ...
        self._hess = None
        self._const = None
        self._ineqconst = None
        self.x = None

    def __call__(self, x, *args):
        self.x = np.asarray(x).copy()
        loss = self.func(x, *args)
        obj = loss[0]
        self._const = loss[1]
        self._ineqconst = loss[2:]
        # self._grad = fgh[1]
        # self._hess = fgh[2]
        return obj

    def const(self, x, *args):
        if self._const is not None and np.alltrue(x == self.x):
            return self._const
        else:
            self(x, *args)
            return self._const

    def ineqconst(self, x, *args):
        if self._ineqconst is not None and np.alltrue(x == self.x):
            return self._ineqconst
        else:
            self(x, *args)
            return self._ineqconst
...
class GradWrapper:
    ...
    def __call__(self, x, *args):
        self.x = np.asarray(x).copy()
        jacob = np.array(self.func(x, *args))
        self._const = jacob[1] # One equality constraint
        self._ineqconst = jacob[2:]
        # self._grad = fgh[1]
        # self._hess = fgh[2]
        return jacob[0]
