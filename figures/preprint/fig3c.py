...
if mode == "just-entire-shell":
    ...
    move_vtx_vec = vtx_to_spider_head * 1.15

    start_bind_idx = vertex_to_bind_idx * 6
    end_bind_idx = start_bind_idx + 6
    shell_body_pos[start_bind_idx:end_bind_idx] -= move_vtx_vec

    abducted_vertex_pos = shell_body_pos[start_bind_idx:end_bind_idx]

    radii = list()
    for i in range(abducted_vertex_pos.shape[0]):
        if i % (num_patches + 1) == 0:
            radii.append(vertex_radius)
        else:
            radii.append(patch_radius)
    ...
    geometry_shell = fresnel.geometry.Sphere(scene, N=abducted_vertex_pos.shape[0], radius=radii)
    geometry_shell.position[:] = abducted_vertex_pos
