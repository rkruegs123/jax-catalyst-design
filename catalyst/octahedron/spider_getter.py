...
class SpiderInfo:
    ...
    def load(self):
        ...
        spider_species = jnp.array([[0] * 4 + [1]]).flatten()
    ...
    def get_positions(self, z=0.0):
        spider_pos = jnp.zeros((4, 3))

        def scan_fn(spider_pos, i):
            x = self.base_radius * jnp.cos(i * 2 * jnp.pi / 4)
            y = self.base_radius * jnp.sin(i * 2 * jnp.pi / 4)
...
class TestSpiderInfo(unittest.TestCase):
    ...
    def test_write_injavis(self):
        ...
        with open('octahedron_spider.pos', 'w+') as of:
            of.write('\n'.join(injavis_lines))
