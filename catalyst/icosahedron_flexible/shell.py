...
class TestShell(unittest.TestCase):
    ...
    def test_load(self):
        ...
        shell = Shell(displacement_fn, shift_fn)

    def test_final_configuration(self):
        displacement_fn, shift_fn = space.free()
        shell = Shell(displacement_fn, shift_fn)
        num_vertices = 12
        body_pos = shell.get_body_frame_positions(shell.rigid_body)
    ...
    def test_energy_fn_no_errors(self):
        displacement_fn, shift_fn = space.free()
        shell = Shell(displacement_fn, shift_fn)
        energy_fn = shell.get_energy_fn()
        init_energy = energy_fn(shell.rigid_body)
