...
class SpiderInfo:
    ...
    def load(self):
        ...
        spider_species = jnp.array([[0] * self.num_legs + [1]]).flatten()
    ...
    def get_positions(self, z=0.0):
        spider_pos = jnp.zeros((self.num_legs, 3))

        def scan_fn(spider_pos, i):
            x = self.base_radius * jnp.cos(i * 2 * jnp.pi / self.num_legs)
            y = self.base_radius * jnp.sin(i * 2 * jnp.pi / self.num_legs)
    ...
    def body_to_injavis_lines(
        ...
        assert(body_pos.shape[0] == self.num_legs+1)
        ...
        for base_idx in range(self.num_legs):
...
class TestSpiderInfo(unittest.TestCase):
    def test_no_errors(self):
        spider_info = SpiderInfo(base_radius=1.5, head_height=4.0,
    ...
    def test_write_injavis(self):
        ...
        injavis_lines, _, _, _ = spider_info.body_to_injavis_lines(spider_rb, box_size=15.0)
        with open('dodecahedron_spider.pos', 'w+') as of:
            of.write('\n'.join(injavis_lines))
