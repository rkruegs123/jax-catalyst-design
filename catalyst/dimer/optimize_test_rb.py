...
if __name__ == "__main__":
    ...
    init_monomers_rb = rigid_body.RigidBody(
    ...
    def loss_fn(params, iter_key):
        ...
        init_fn, step_fn = simulate.nvt_langevin(energy_fn, shift_fn, dt=dt, kT=1.0, gamma=gamma)

        mass = rigid_body.RigidBody(center=jnp.array([1.0]), orientation=jnp.array([1.0]))
        init_state = init_fn(iter_key, init_monomers_rb, mass=mass)
        ...
        def fori_step_fn(t, iter_info):
            ...
            state = step_fn(state)
            dist = space.distance(displacement_fn(state.position.center[0], state.position.center[1]))
